...
## These code was modified based on the module of features.py in ogb (https://ogb.stanford.edu/docs/home/)
...
def atom_to_feature_vector_base(atom):
    """
...
def atom_to_feature_vector_minimalism(atom):
    """
    Converts rdkit atom object to feature list of indices
    :param mol: rdkit atom object
    :return: list
...
def atom_to_one_hot_feature_vector_base(atom):
    atom_feature = []
    atom_feature += one_of_k_encoding_unk(
                atom.GetAtomicNum(), allowable_features['possible_atomic_num_list']
                )
    atom_feature += one_of_k_encoding_unk(
                atom.GetChiralTag(), allowable_features['possible_chirality_list']
                )
    atom_feature += one_of_k_encoding_unk(
                atom.GetTotalDegree(), allowable_features['possible_degree_list']
                )
    atom_feature += one_of_k_encoding_unk(
                atom.GetFormalCharge(), allowable_features['possible_formal_charge_list']
                )
    atom_feature += one_of_k_encoding_unk(
                atom.GetTotalNumHs(), allowable_features['possible_numH_list']
                )
    atom_feature += one_of_k_encoding_unk(
    ...
    atom_feature += one_of_k_encoding_unk(
                atom.IsInRing(), allowable_features['possible_is_in_ring_list']
                )
    return atom_feature

def atom_to_one_hot_feature_vector_minimalism(atom):
...
ATOM_FAMILIES = ['Acceptor', 'Donor', 'Hydrophobe', 'Aromatic', 'LumpedHydrophobe', 
                 'NegIonizable', 'PosIonizable', 'ZnBinder']
ATOM_FAMILIES_ID = {s: i for i, s in enumerate(ATOM_FAMILIES)}
def mol_to_graph_feature(rdmol, one_hot=True):
    atom_feat_mat_1 = np.zeros([rdmol.GetNumAtoms(), len(ATOM_FAMILIES)], dtype=np.int64)
    for feat in FACTORY.GetFeaturesForMol(rdmol):
        atom_feat_mat_1[feat.GetAtomIds(), ATOM_FAMILIES_ID[feat.GetFamily()]] = 1
    if one_hot:
        atom_feat_mat_2 = np.array([atom_to_one_hot_feature_vector_base(a) for a in rdmol.GetAtoms()], dtype=np.int64)
    else:
        atom_feat_mat_2 = np.array([atom_to_feature_vector_base(a) for a in rdmol.GetAtoms()], dtype=np.int64)
    atom_feat_mat = np.concatenate([atom_feat_mat_2, atom_feat_mat_1], axis=1)
    ...
    if one_hot:
        for b in rdmol.GetBonds():
            ...
            bond_feat = bond_to_one_hot_feature_vector(b)
        ...
        for b in rdmol.GetBonds():
            ...
            col = [b.GetEndAtomIdx(), b.GetBeginAtomIdx()]
            edge_index.extend([row, col])
            bond_feat = bond_to_feature_vector(b)
            edge_attr.extend([bond_feat, bond_feat])
    return atom_feat_mat, np.array(edge_index, dtype=np.int64).T, np.array(edge_attr, dtype=np.int64)


def mol_to_graph_feature_minimalism(rdmol, one_hot=True):
    atom_feat_mat_1 = np.zeros([rdmol.GetNumAtoms(), len(ATOM_FAMILIES)], dtype=np.int64)
    for feat in FACTORY.GetFeaturesForMol(rdmol):
        atom_feat_mat_1[feat.GetAtomIds(), ATOM_FAMILIES_ID[feat.GetFamily()]] = 1
    if one_hot:
        atom_feat_mat_2 = np.array([atom_to_one_hot_feature_vector_minimalism(a) for a in rdmol.GetAtoms()], dtype=np.int64)
    else:
        atom_feat_mat_2 = np.array([atom_to_feature_vector_minimalism(a) for a in rdmol.GetAtoms()], dtype=np.int64)
    atom_feat_mat = np.concatenate([atom_feat_mat_2, atom_feat_mat_1], axis=1)
    ...
    if one_hot:
        for b in rdmol.GetBonds():
            ...
            col = [b.GetEndAtomIdx(), b.GetBeginAtomIdx()]
            edge_index.extend([row, col])
            bond_feat = bond_to_one_hot_feature_vector_minimalism(b)
            edge_attr.extend([bond_feat, bond_feat])
    else:
        for b in rdmol.GetBonds():
            row = [b.GetBeginAtomIdx(), b.GetEndAtomIdx()]
            col = [b.GetEndAtomIdx(), b.GetBeginAtomIdx()]
            edge_index.extend([row, col])
            bond_feat = bond_to_feature_vector_minimalism(b)
            edge_attr.extend([bond_feat, bond_feat])
    return atom_feat_mat, np.array(edge_index, dtype=np.int64).T, np.array(edge_attr, dtype=np.int64)
...
def bond_to_feature_vector_minimalism(bond):
    """
    Converts rdkit bond object to feature list of indices
    :param mol: rdkit bond object
    :return: list
...
def bond_to_one_hot_feature_vector(bond):
    """
    Converts rdkit bond object to feature list of indices
    :param mol: rdkit bond object
    :return: list
    """
    bond_feature = []
    bond_feature += one_of_k_encoding_unk(str(bond.GetBondType()), allowable_features['possible_bond_type_list'])
    bond_feature += one_of_k_encoding_unk(str(bond.GetStereo()), allowable_features['possible_bond_stereo_list'])
    bond_feature += one_of_k_encoding_unk(bond.GetIsConjugated(), allowable_features['possible_is_conjugated_list'])
    return bond_feature


def bond_to_one_hot_feature_vector_minimalism(bond):
    """
    Converts rdkit bond object to feature list of indices
    :param mol: rdkit bond object
    :return: list
    """
    return one_of_k_encoding_unk(str(bond.GetBondType()), allowable_features['possible_bond_type_list'])
